...
| `no-omega` ablation ran a different memory | `neural_memory.py`, `experiments/configs.py` | New `per_token_updates` flag (2026-09-02): the per-token store path (per-token gradients at segment-start weights, per-token gates, η outside Newton-Schulz, per-token retrieve) was selected by `omega_context > 1`, so the no-omega ablation silently fell back to lucidrains' chunk-wise path — per-chunk gradients and retrieve, pooled gates, η washed out by Muon — five changes for one ablation. `atlas_config` now sets the flag; `no-omega` is `omega_context=1` and differs from `atlas` by the window and its gates only (equivalence and perturbation guards in `tests/test_titans.py`) |
