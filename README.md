...
It is a working, paper-faithful port. The three Atlas additions (the Omega Rule, polynomial feature mapping, and Muon-style Newton-Schulz orthogonalization) are implemented and checked against the paper, with runtime tests in `tests/test_titans.py` that assert the paper-mandated transformations actually fire. Several correctness bugs in the underlying memory code are also fixed. The table below lists exactly what was added and what was fixed.
...
| Muon / Newton-Schulz | `neural_memory.py` | `newtonschulz5()` — 5-iteration Newton-Schulz orthogonalization of the surprise updates (Section 5, Eq 32) |
...
|-----|------|-------------|
| Per-head memory parameters aliased one tensor | `neural_memory.py` | `einops.repeat` built the per-head copies of the memory-MLP parameters as a stride-0 view, so every head shared one storage and `load_state_dict` failed on any checkpoint with `heads > 1` (PR #14, 2026-04-25). Each head now owns a cloned copy, guarded by a regression test on the stride. The same bug is open upstream: diagnosis in [lucidrains/titans-pytorch#64](https://github.com/lucidrains/titans-pytorch/issues/64), fix proposed in [#65](https://github.com/lucidrains/titans-pytorch/pull/65) |
...
| Per-token path guards | `neural_memory.py` | Assertions preventing the per-token store path (and so omega) with `num_kv_per_token > 1`, `store_with_lookahead_value` or `accept_weight_residual`; `ValueError` for `per_token_retrieve` without the per-token path and for a window without it |
...
| `no-omega` ablation ran a different memory | `neural_memory.py`, `experiments/configs.py` | New `per_token_updates` flag (2026-09-02): the per-token store path (per-token gradients at segment-start weights, per-token gates, η outside Newton-Schulz, per-token retrieve) was selected by `omega_context > 1`, so the no-omega ablation silently fell back to lucidrains' chunk-wise path — one summed gradient per store chunk, pooled gates, η folded into the gradient (its magnitude normalized away by Muon on matrix parameters), per-chunk scan and retrieve — five changes for one ablation; both paths share the segment-start base weights. `atlas_config` now sets the flag; `no-omega` is `omega_context=1` (the paper's `c = 1` ablation row) and differs from `atlas` by the window and its gates only (equivalence and perturbation guards in `tests/test_titans.py`). `model_config.json` gains the key, so the eval drift guard refuses checkpoints recorded before it |
...
config = NeuralMemory.atlas_config()          # Omega Rule + polynomial features + Muon-orthogonalized surprise
...
This repository is an implementation and a set of measurements at one small scale. In September 2026 I trained five 170M-parameter arms on 1B tokens of FineWeb at a 1K training context, identical data order and recipe: the Atlas memory, the same memory without the Omega window, lucidrains' chunk-wise Titans memory at two learning rates, and a memory-free control. What those runs support, and what they do not, is set out below. They say nothing about Atlas at the paper's scale; they do say specific things at this one.

**Validation loss on held-out FineWeb** (nats, lower is better): Atlas 3.4003, no-window 3.3965, chunk-wise Titans 3.3949, memory-free control 3.4560, Titans at 4e-4 3.7676. Having a memory is worth about 0.06 nats; the learning rate between the two Titans arms is worth 0.37, six times more. Which update rule runs it is not distinguishable at this scale, and neither is the Omega window.

**Extrapolation past the training length.** Per-position loss over a fixed text span evaluated at 1K, 2K, 4K and 8K sequence lengths: the two per-token configurations are flat (Atlas 3.304 / 3.294 / 3.292 / 3.296) and beat the memory-free control at every length, while the chunk-wise configuration degrades from 2K and overflows at 8K. A probe of the memory state norm attributes that to one memory layer's state growing to about 2e9 and injecting values of that size into the residual stream; the per-token configurations' states stay bounded and decay. Whole-sequence and chunked forwards agree, so this is the model, not the inference path.

**Long-range retrieval: none measurable.** A synthetic needle probe (`eval/diagnostics/needle_recall.py`) places a random 12-token needle, then filler of a chosen length, then the needle's first 4 tokens, and scores the remaining 8. Inside the 64-token attention window every arm recalls strongly and the memory arms beat the control by 0.6 to 1.2 nats. At 128 tokens and beyond, every arm's margin collapses to about 0.1 nats, indistinguishable from the memory-free control, out to 4096 tokens. On BABILong the memory-free control matches the memory arms at every context length of 2K and above, which is why that benchmark cannot discriminate here. At this scale the memory helps local prediction and carries nothing recoverable past the attention window.

Five known limitations are worth reading before building on this at scale:

1. **Chunk-wise divergence past the training length.** The chunk-wise memory configuration diverges numerically past its training length (above).
2. **Polynomial memory capacity.** The polynomial memory capacity is bounded under the default configuration (tracked in the [issues](https://github.com/danielquintas8/atlas-torch/issues)).
3. **Autograd memory.** The per-token gradient path is incompatible with gradient checkpointing, which caps how many memory layers fit on a single GPU.
4. **Parallel-vs-decoding mismatch.** Token-by-token decoding (`model.sample()`) runs a different memory than the parallel forward. With per-token retrieve (the Atlas configuration, `omega_context > 1`) decoding reads the state after the last store-cache flush instead of each token's own post-update state, so it deviates from the first token at any batch size (3.7 max logit deviation at `omega_context=8` with the batch equal to the store chunk, toy geometry, fp64); on top of that, omega windows truncate at flush boundaries during decoding but span the full neural-memory batch segment in parallel mode. On the `omega_context=1` path the remaining difference is the key/value short convs, which zero-pad at every flush while the parallel forward pads once per memory segment whenever `neural_memory_batch_size` exceeds `chunk_size` (2.6 max logit deviation at batch size 64 in the same toy geometry, 1.27 in an earlier one; exactly 8.9e-16 when the batch size equals the store chunk). The query short conv is not part of the mismatch: the carried `retrieve_conv_cache` gives decoding the same previous query rows the parallel forward sees. Training, likelihood-style evaluation (e.g. BABILong scoring) and chunked inference use the parallel forward's segmenting and are unaffected.
5. **Evaluation memory ceiling.** With per-token retrieve (the Atlas config), the whole-sequence forward materializes every token's memory-weight state for the final retrieve — at 170M that is 0.59 MB per token per memory layer in bf16 (1.18 MB per token for the two shipped layers; double in fp32): 4.8 GB at 4K, 19 GB at 16K, 39 GB at 32K, 78 GB at 64K and 1.2 TB at 1M for the retrieve state alone — and the store path holds several per-token-sized copies on top of it at once (a live-tensor high-water tracker measured the forward peaking at 2.8–10× the retrieve state across toy geometries and lengths, highest within one memory segment and where the attention path is a large share of the total; with the attention path subtracted the memory-only ratio is 3.7–5.6× within one segment and 1–1.6× at 4–8 segments). `eval/babilong/evaluate.py` estimates the peak before each length as a flat 8× the retrieve state of the positions processed at once plus a separate attention/residual term, re-checks that estimate against live measurements in `tests/test_babilong_scorer.py` (never under, at most 5× over), and refuses to run past 80% of the device's memory unless `--force` is given. On a 64 GB H100 at 170M with two memory layers that estimate allows roughly 5K tokens in bf16 (about 2.5K in fp32); the measured ratios would allow two to three times more, but chunked inference makes the whole-sequence cap moot.

   **Chunked inference removes the ceiling**: `MemoryAsContextTransformer.iter_chunked_hidden` / `forward_chunked` (and `--chunk-len` in the evaluator and the NLL diagnostic) process the sequence in chunks of `chunk_len` *interleaved* positions — a multiple of `neural_memory_batch_size`, so chunk boundaries coincide with the memory's own segment boundaries — carrying only the memory state and each attention layer's un-rotated keys/values for the last two attention segments across chunks. Each chunk is folded into attention segments exactly as the whole-sequence forward folds the full sequence, so attention memory is linear in the chunk. The output equals the whole-sequence forward (fp64-exact parity tests for the Atlas, Titans, memory-free, axial-embedding and block-attention configurations) and peak memory is O(chunk): about 10 GB by the estimate (about 6 GB at the measured ratios) at 170M in bf16 with 1024-position chunks, at any context length. One API change supports it: `NeuralMemory.retrieve_memories` now returns `(retrieved, next_conv_cache)` instead of the tensor alone, and `NeuralMemState` carries a sixth field, `retrieve_conv_cache` (default `None`), holding the previous call's last pre-conv query rows.
